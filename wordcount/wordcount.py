...
def process_text(text: bytes) -> Dict[str, int]:
    ...
    the text to merge cases and plurals into single value.
    """
...
def process_tokens(words: List[str]) -> Dict[str, int]:
    ...
    # keys of d are word.lower()
    # values are dicts counting frequency of each capitalization
    ...
    for key in list(d.keys()):
        if key.endswith('s') and not key.endswith('ss'):
            ...
            if key_singular in d:
                ...
                merge_plurals[key] = key_singular
                del d[key]
